...
void spells(playerInfo *player,enemyInfo *enemy, int &);
...
int main (int argc, char **argv){
    ...
    cout<<"Error "<<d<<" is not a choice"<<endl;
    }
    maxInfo max(j,s,m,h);
...
void combat(maxInfo max,playerInfo *player,enemyInfo *enemy,int &en, bool &out){
    ...
    while(enemy->enHlth[en]>0&&player->health!=0&&out==false){
        ...
        cout<<"Mana:"<<player->mana<<"/"<<max.getMana()<<endl;
        if(max.getHlth()==150){
            cout<<"A->Attack\nD->Run"<<endl;
            cout<<"S->Scan"<<endl;
            cin>>d; 
...
void spells(playerInfo *player,enemyInfo *enemy,int &en){
    char d;
    cout<<"Would you like to use Fire, Thunder, or Healing Magic?"<<endl;
    cout<<"A->Fire"<<endl;
    cout<<"S->Thunder"<<endl;
    cout<<"D->Heal"<<endl;
    cin>>d;
...
void win(){
    cout<<"You escape the small dungeon and see a city out in the distance"<<endl;
